    ...
    public ModelAndView loginWeChat(HttpServletRequest request, HttpSession httpSession, HttpServletResponse response) {
        ...
        try {
            ...
            if (!CoreStringUtils.isEmpty(code)) {
                String access_token = CoreHttpUtils.get("https://api.weixin.qq.com/sns/oauth2/access_token?appid=wx2d4606a7e7af7ba9&secret=e983561966ad58e8885f924505886053&code=" + code + "&grant_type=authorization_code", "utf-8");
...
//            return new ModelAndView("redirect:/user/person");
            response.sendRedirect("http://rchyxz.chuangmikeji.com/");
        } catch (Exception e) {
            e.printStackTrace();
        }
