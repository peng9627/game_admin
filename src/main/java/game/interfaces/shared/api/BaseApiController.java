package game.interfaces.shared.api;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.serializer.SerializerFeature;
import game.core.api.ApiResponse;
import game.core.exception.ApiAuthenticationException;
import game.core.util.CoreStringUtils;
import game.core.util.RSAUtils;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.security.Key;

/**
 * Created by pengyi on 2016/4/15.
 */
public abstract class BaseApiController {

    protected Logger logger = LoggerFactory.getLogger(BaseApiController.class);

    protected <T> T authenticationAndConvert(HttpServletRequest request, Class<T> clz) throws ApiAuthenticationException {

        String content = "";
        if (request.getParameterMap().keySet().iterator().hasNext()) {
            content = request.getParameterMap().keySet().iterator().next();
        }

        try {
            if (!CoreStringUtils.isEmpty(content)) {
                Key privateKey = RSAUtils.getPrivateKey("");
                content = new String(RSAUtils.decrypt(privateKey, content), "utf-8");
                if (!CoreStringUtils.isEmpty(content)) {
                    return JSON.parseObject(content, clz);
                }
            }
            return null;
        } catch (Exception e) {
            e.printStackTrace();
            throw new ApiAuthenticationException();
        }
    }

    protected void returnData(HttpServletResponse response, ApiResponse apiResponse) {
        response.setCharacterEncoding("utf-8");
        SerializerFeature[] features = new SerializerFeature[]{SerializerFeature.WriteNullListAsEmpty,
                SerializerFeature.WriteMapNullValue, SerializerFeature.DisableCircularReferenceDetect,
                SerializerFeature.WriteNullStringAsEmpty, SerializerFeature.WriteNullNumberAsZero,
                SerializerFeature.WriteNullBooleanAsFalse};
        int ss = SerializerFeature.config(JSON.DEFAULT_GENERATE_FEATURE, SerializerFeature.WriteEnumUsingName, false);

        try {
            Key privateKey = RSAUtils.getPrivateKey("");
            byte[] bytes = RSAUtils.encrypt(privateKey, JSON.toJSONString(apiResponse, ss, features));
            if (bytes != null) {
                response.getOutputStream().write(bytes);
            }
        } catch (IOException e) {
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}
