package game.interfaces.auth.web;

import game.application.account.representation.AccountRepresentation;
import game.application.auth.IAuthAppService;
        ...
        if (flag) {
            try {
                ...
                if (subject.hasRole("admin")) {
                    ...
                    logger.info(subject.getPrincipal() + "登录成功！时间:" + new Date());
                    session.setAttribute(Constants.SESSION_USER, user);
                    return new ModelAndView("redirect:/user/children?parent=" + user.getId());
                } else if (subject.hasRole("agent")) {
                    logger.info(subject.getPrincipal() + "登录成功！时间:" + new Date());
                    session.setAttribute(Constants.SESSION_USER, user);
                    return new ModelAndView("redirect:/user/agent");
                } else if (subject.hasRole("finance")) {
                    logger.info(subject.getPrincipal() + "登录成功！时间:" + new Date());
                    session.setAttribute(Constants.SESSION_USER, user);
                    return new ModelAndView("redirect:/withdraw/finance/pagination?status=PENDING");
                } else {//用户没有对应角色 不让登录
